...
int ZEXPORT unzGetSubFiles(unzFile file, const char* szDir, char** subPaths, int* count) {
	unz64_s* s;
	int err;
	...
	if (strlen(szDir) >= UNZ_MAXFILENAMEINZIP)
		return UNZ_PARAMERROR;
	...
	*count=0;
	...
	while (err == UNZ_OK) {
		...
		err = unzGetCurrentFileInfo64(file, NULL,
									  ...
									  NULL, 0, NULL, 0);
		if (err == UNZ_OK) {
			if (strstr(szCurrentFileName, szDir) == szCurrentFileName) {
				strcpy(subPaths[*count++], szCurrentFileName);
			}
			err = unzGoToNextFile(file);
		}
	}
